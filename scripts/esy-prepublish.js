...
const head = cp
  ...
  .stdout.toString()
const master = cp
  .spawnSync('git', ['rev-parse', '--verify', 'master'])
  .stdout.toString()

let uncommitted = cp
  .spawnSync('git', ['diff-index', 'HEAD', '--'])
  .stdout.toString()

if (uncommitted !== '') {
  console.log(
    'ERROR: You have uncommitted changes. Please try on a clean master branch'
  )
  process.exit(1)
}

process.chdir(projectRoot)
let tarResult = cp.spawnSync('tar', [
  '--exclude',
  'node_modules',
  '--exclude',
  '_build',
...
try {
  ...
  for (var i = 0; i < relativeJsonPaths.length; i++) {
    ...
    if (fs.existsSync(subpackageReleaseDir)) {
      console.log(
        'YOU NEED TO REMOVE THE ' + subpackageReleaseDir + ' DIR FIRST!'
      )
      process.exit(1)
    }
    ...
    let subpackageReleasePrepDir = path.resolve(
      _releaseDir,
      path.join(jsonRelativePath),
      '_prep'
    )
    ...
    const packResult = cp.spawnSync(
      process.platform === 'win32' ? 'npm.cmd' : 'npm',
      ['pack']
    )
    if (packResult.status !== 0) {
      console.log(
        'ERROR: Could not create npm pack for ' + subpackageReleasePrepDir
      )
      throw new Error('Error:' + packResult.stderr.toString())
    }
    const mvTo = subpackageReleaseDir
    fs.readdirSync(subpackageReleasePrepDir)
    ...
    console.log(
      packageName +
        '@' +
        packageVersion +
        ' prepared for publishing at ' +
        subpackageReleaseDir
    )
    console.log('')
